...
class EnhancedGooglePlacesWidget extends StatefulWidget {
  ...
  @override
  State<EnhancedGooglePlacesWidget> createState() => _EnhancedGooglePlacesWidgetState();
}
...
class _EnhancedGooglePlacesWidgetState extends State<EnhancedGooglePlacesWidget>
  ...
  void initState() {
    ...
    _fadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _animationController, curve: widget.animationCurve ?? Curves.easeInOut),
    );
  ...
  void _onTextChanged() {
    ...
    // Debounce the search
    _debounceTimer = Timer(widget.debounceDelay ?? const Duration(milliseconds: 300), () {
      _searchPlaces(text);
    });
  }
  ...
  void _showOverlayInternal() {
    ...
    final offset = widget.suggestionOffset ?? Offset(0, size.height + (widget.suggestionSpacing ?? 4));
    ...
    _overlayEntry = OverlayEntry(
      builder: (context) => CompositedTransformFollower(
        ...
        child: Material(
          ...
          borderRadius: widget.suggestionBorderRadius ?? BorderRadius.circular(8),
          color: widget.suggestionBackgroundColor ?? Theme.of(context).cardColor,
  ...
  Widget _buildSuggestionsList(double width) {
    if (_predictions.isEmpty) {
      return widget.noResultsWidget ?? Container(
        width: width,
        padding: const EdgeInsets.all(16),
        child: Text(
          'No results found',
          style: widget.suggestionSubtextStyle ?? TextStyle(color: Colors.grey[600]),
    ...
    return Container(
      ...
      constraints: BoxConstraints(
        maxHeight: widget.maxSuggestionHeight ?? 300,
      ),
  ...
  Widget _buildDefaultSuggestionItem(PlacePrediction prediction, int index) {
    return InkWell(
      ...
      child: Container(
        ...
        padding: widget.suggestionPadding ?? const EdgeInsets.symmetric(
          horizontal: 16,
          vertical: 12,
        ),
        ...
        child: Row(
          children: [
            Padding(
              ...
              child: widget.locationIcon ?? Icon(
                Icons.location_on,
              ...
              child: Column(
                ...
                children: [
                  if (prediction.mainText != null)
                    Text(
                      prediction.mainText!,
                      style: widget.suggestionTextStyle ?? const TextStyle(
                  ...
                  if (prediction.secondaryText != null)
                    Text(
                      ...
                      style: widget.suggestionSubtextStyle ?? TextStyle(
                  ...
                  if (prediction.mainText == null)
                    Text(
                      ...
                      style: widget.suggestionTextStyle ?? const TextStyle(fontSize: 14),
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                    ),
                ],
  ...
  Widget _buildSuffixIcon() {
    if (_isLoading && widget.showLoadingIndicator) {
      return widget.loadingWidget ?? Padding(
    ...
    if (_controller.text.isNotEmpty && widget.showClearButton) {
      return widget.clearIcon ?? IconButton(
    ...
    Widget child = CompositedTransformTarget(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_errorMessage != null && widget.errorBuilder == null && widget.showInlineErrors)
            Padding(
